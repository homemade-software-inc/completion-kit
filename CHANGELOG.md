...
### Added
- **A JSON field can now be graded against each row's own answer key.** `compare_to: "expected"` was available on `contains`, `not_contains` and `equals` only, so `json_path_equals` could compare a field against a constant but never against the ground truth for that row. That is fine when a row has one answer and falls apart when the output is a record. An extraction returning a VIN, a trim, a set of option codes and two colours from one photo had to be pushed as one field per dataset, which multiplies rows by fields and loses the fact that they came from the same photo, or graded on a single field with the rest computed outside CompletionKit and given no history. `json_path_equals` now accepts `compare_to: "expected"`, with `expected_path` selecting the matching branch of the answer key, so a record is gradeable field by field with one metric per field over one dataset. The answer key keeps its own type on the way through, so a JSON `200` matches the number 200 rather than failing against the string.

- **A new `list_overlap` check scores a list-valued field as a fraction rather than a verdict.** Every existing check answers yes or no about one value, but a large class of extraction output is a list, and the useful measure over a list is partial. A row expecting six option codes that gets four back was previously called a failure, the same verdict given to a run that returned zero and to one that returned five. `list_overlap` compares two lists and reports `recall`, `precision`, `f1` or `jaccard`, chosen with `score_by`. The fraction is stored on the review in a new `check_score` column, and each run averages it per metric as `avg_check_score`, which is exposed in `Run#as_json` next to `check_pass_rate` so a pipeline can gate on it. Its `min` is the fraction that counts as a pass, defaulting to an exact match. This shape is not specific to option codes; tags, entities, features, ingredients, symptoms and citations all extract the same way.

- **Two new numeric checks, `numeric_bounds` and `numeric_equals`.** `length_bounds` takes a min and a max but measures the length of a string, so there was no way to assert that an extracted number falls in a range or matches the row's expected number within a tolerance. That blocked two things. A per-field confidence could not be bounded, so a grade could not separate right and sure from right by luck, and could say nothing about whether the confidence a ranking rests on is calibrated at all. Extracted figures fared no better: `equals` on "82,000" against "82000" fails on formatting rather than on being wrong, and a model year out by one was graded the same as one out by forty. Both new kinds read a number, ignoring the commas, spaces and leading currency symbol a figure is written with. `numeric_equals` takes a `tolerance` that is a plain number for a fixed amount, or a percentage string such as `"2%"` for a share of the expected value.

### Fixed
- **A path with an array index never resolved, though the form has always advertised one.** The hint under "Path into the JSON" offers `data.items.0.name` as an example, but path resolution stopped at the first node that was not an object, so any path stepping into a list returned nothing and every row of the run reported "could not resolve target". Numeric segments now index into a list, and a segment that is out of range or not a number still resolves to nothing rather than to the wrong element. A `target_path` saved with stray whitespace is also trimmed now, matching what `expected_path` already did, instead of saving cleanly and then never matching.

- **A regex check could never be made case-insensitive from the form.** The "Case sensitive" box submitted nothing when unchecked, so the key was simply absent from the saved config, and `regex` was the one kind that read an absent key as case-sensitive while every other kind read it as case-insensitive. The box therefore misreported what the check did, and the case-insensitive setting it appeared to offer was unreachable. The three check boxes now submit an explicit `false` when unchecked, and `regex` reads `case_sensitive` the same way as every other kind. A regex metric saved before this release with no explicit `case_sensitive` therefore changes from matching case-sensitively to matching case-insensitively; set the box to restore the old behaviour.

- **`case_sensitive`, `multiline` and `trim` are now read as booleans however they were written.** The checks compared against the literal boolean `true`, but the REST API stores `check_config` exactly as it is posted with no coercion, so a JSON client sending `"case_sensitive": "true"` got the opposite of what it asked for. The string forms, and the `1` and `0` forms, are now coerced the way the rest of Rails coerces them.

- **The "Trim whitespace" box did nothing on a contains or does-not-contain check.** The form offered it for all three text kinds but only `equals` implemented it, so on the other two it was a silent no-op that quietly changed nothing.

- **A length check with both bounds left blank could never fail.** An empty string is not nil, so a blank bound was read as the number zero, and a length is never below zero. Such a config also passed validation, because validation only rejected the case where both bounds were nil. A blank bound is now treated as absent, and a bound that is not a number is rejected at authoring time rather than silently becoming zero.

### Changed
- **Saving a check no longer keeps the fields belonging to a kind you moved away from.** The form submits every field it has, visible or not, so switching from a JSON check to a regex check left the old `json_path` and `expected` in the saved config, where the metric page then listed them as though the check read them. A saved check now keeps only the keys its own kind uses, plus the shared ones.
