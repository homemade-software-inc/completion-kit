...
RSpec.describe CompletionKit::ApplicationHelper, type: :helper do
  ...
  describe "check label helpers" do
    ...
    it "never shows the scoring jargon the stored value uses" do
      expect(helper.ck_check_field_value("score_by", "recall")).to eq("How much of the expected list was found")
      expect(helper.ck_check_field_value("score_by", "f1")).to eq("A balance of the two")
      expect(helper.ck_check_field_value("score_by", "jaccard")).to eq("How much the two lists overlap")
    end

    it "passes an unrecognised measure through rather than blanking it" do
      expect(helper.ck_check_field_value("score_by", "made_up")).to eq("made_up")
      expect(helper.ck_check_field_value("value", "made_up")).to eq("made_up")
    end

    it "knows which config fields need translating on the metric page" do
      expect(helper.ck_check_field_translated?("score_by")).to be(true)
    ...
    it "offers the plain-language options the form renders" do
      expect(helper.ck_check_score_by_options.map(&:first)).to eq(CompletionKit::Checks::ListOverlap::MEASURES)
      expect(helper.ck_check_score_by_options.map(&:last)).to all(match(/\A[A-Z]/))
      expect(helper.ck_check_score_by_options.map(&:last).join).not_to match(/recall|precision|jaccard|F1/i)
    end

    it "labels the new numeric and list fields without engineering jargon" do
      expect(helper.ck_check_field_label("score_by")).to eq("How to score it")
