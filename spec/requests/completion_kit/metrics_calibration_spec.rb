...
RSpec.describe "CompletionKit metrics (calibration surfaces)", type: :request do
  ...
  describe "GET metric show — disagreements section" do
    it "renders the empty state when no disagreements exist" do
      ...
      expect(response.body).to include("Disagreements")
      expect(response.body).to include("No disagreements yet")
    end
    ...
    it "lists disagreements with judge + human scores and an Add-as-few-shot button" do
      ...
      get "/completion_kit/metrics/#{metric.id}"
      expect(response.body).to include("Add as judge few-shot")
      expect(response.body).to include("off by a star")
      expect(response.body).to include("row ##{r1.id}")
    end

    it "shows an 'Added' chip and hides the button once a disagreement has been promoted to a few-shot" do
      r1 = create(:completion_kit_response, run: run)
      add_review(r1, ai_score: 5.0)
      ...
      expect(response.body).not_to include("Add as judge few-shot")
    end
    ...
    it "hides the section when judge_calibration_enabled is off" do
      ...
      get "/completion_kit/metrics/#{metric.id}"
      expect(response.body).not_to include("Disagreements")
    ensure
  ...
  describe "POST add_few_shot" do
    it "appends the calibration's context to the metric's few_shot_examples and redirects with a flash" do
      ...
      follow_redirect!
      expect(response.body).to include("Added as a judge few-shot")
      metric.reload
